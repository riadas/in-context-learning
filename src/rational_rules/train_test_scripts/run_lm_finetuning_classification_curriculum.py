...
train_dataset_paths = [] # sys.argv[0]
test_dataset_paths = [] # sys.argv[1]
...
class RationalRulesDataset(Dataset):
  ...
  def __init__(self, paths, use_tokenizer):
    ...
    for path in paths: 
      ...
      lines = list(filter(lambda l: l != "", content.split("\n")))
...
if __name__ == "__main__":
  ...
  for i in range(train_dataset_paths):
    ...
    # Create pytorch dataset.
    valid_dataset =  RationalRulesDataset(path=test_dataset_paths[:i+1], 
                                  use_tokenizer=tokenizer)
